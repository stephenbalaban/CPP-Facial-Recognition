...
// Global variables
IplImage ** faceImgArr        = 0; // array of face images (array of arrays)
...
int main( int argc, char** argv )
{
	if( argc >= 2 && strcmp(argv[1], "single") == 0) {
		...
		if (argc == 3) {
			szFileRecognize = argv[2];
		} else {
			printf("ERROR: No image file given.\n");
			return 1;
		}
		recognizeFromFilename(szFileRecognize);
	}
...
{
	return faceRect.width > 0;
}

// greyscaleImage, faceRect -> cropped and resized on face
IplImage* cropAndResize(IplImage* greyscaleImage, CvRect faceRect)
{
	IplImage* sizedImg;
	IplImage* croppedImg;
	// crop on face //
 	croppedImg = cropImage(greyscaleImage, faceRect);	// Get the detected face image.
	// resize image //
	// faceWidth, faceHeight are globals
	sizedImg = resizeImage(croppedImg, faceWidth, faceHeight);
...
IplImage* normalizeImage(IplImage* sizedImg)
{
	...
	if ( !equalizedImg )
	{
		fprintf(stderr, "Failed cvEqualizeHist, no input image found!");
		exit(1);
	}
	return equalizedImg;
}

// greyscaleImage, faceRect -> normalizedImg
IplImage* cropAndNormalize(IplImage* greyscaleImage, CvRect faceRect)
{
	IplImage* processedFaceImg;
	IplImage* sizedImg;
	// resize image
	sizedImg = cropAndResize(greyscaleImage, faceRect);
	// normalize image brightness / contrast
	processedFaceImg = normalizeImage(sizedImg);

	cvReleaseImage( &sizedImg );
	return processedFaceImg;
}

// nEigens is a global as is eigenVectArr and pAvgTrainImg
int idFromNormalizedImg(IplImage* normalizedImg, float* pConfidence)
{
	...
	cvEigenDecomposite(
			   normalizedImg,
			   nEigens, // need to make sure this is correct
			   eigenVectArr,
			   0, 0,
			   pAvgTrainImg,
			   projectedTestFace);
...
// raw img -> user id in local database
int recognizeFromFilename(const char *rawImgFilename)
{
	int resultId;
	float confidence;
	IplImage *greyscaleImg;
	IplImage *normalizedImg;
	CvRect faceRect;

	// Load Image
	greyscaleImg = cvLoadImage(rawImgFilename, CV_LOAD_IMAGE_GRAYSCALE);
	if ( !greyscaleImg )
       	{
	...
	// Check if a face is found (refactor)
	faceRect = detectFaceInImage(greyscaleImg, faceCascade);
	if (!faceFound(faceRect)) 
	{
		fprintf(stderr, "No faces found in file %s\n", rawImgFilename);
		return -1;
	}
